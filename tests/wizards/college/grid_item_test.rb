class Tests_Wizards_College_GridItemTest < Lib_Tools_GridViewItemSpecification   
  ...
  def canViewProfile
    Proc.new do
      begin    
        gridImage.click
        raise "verify redirect to profile" if not @watir_helper.urlsLike("#{$target_server}/college")
        @watir_helper.reset.link(:text => /Back to search/).click
        raise "verify redirect to wizard" if not @watir_helper.urlLike("#{$target_server}/find/college")
        _exit = true
      rescue Exception => e
        puts "FAILED to #{e}"
        _exit = false
