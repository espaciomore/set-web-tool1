class Tests_Wizards_Medical_GridItemTest < Lib_Tools_GridViewItemSpecification
  ...
  def hasDescription2
    return description2.getText.match(/((((\w[A-Za-z .]+), ([A-Z][A-Z]))|(Online)))+/)!=nil
  end  
  ...
  def canViewProfile
    gridImage.click
    profile = @watir_helper.urlLike "#{$target_server}/medical"
    @watir_helper.reset.link(:text => /Back to search/).click
    wizard = @watir_helper.urlLike "#{$target_server}/medical/search"
