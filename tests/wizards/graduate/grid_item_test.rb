class Tests_Wizards_Graduate_GridItemTest < Lib_Tools_GridViewItemSpecification
  ...
  def hasDescription2
    Proc.new do
      begin
        raise "verify location grammar is correct" if not description2.getText.match(/(([A-Za-z .]+), ([A-Z][A-Z]))+/)!=nil     
        _exit = true
      rescue Exception => e
        puts "FAILED to #{e}"
        _exit = false
  ...
  def canViewProfile
    Proc.new do
      begin
        gridImage.click
        raise "verify redirect to profile" if not @watir_helper.urlLike( "#{$target_server}/graduate" )
        @watir_helper.reset.link(:text => /Back to search/).click
        raise "verify redirect to wizard" if not @watir_helper.urlLike( "#{$target_server}/find/graduate" )
        _exit = true
      rescue Exception => e
        puts "FAILED to #{e}"
        _exit = false
